...
used. In the example below the version tag `0.6.2` is specified.
...
tag** `0.6.2` which is the current version.
...
```typescript
https://deno.land/x/deno_mod@0.6.2/mod.ts
```
...
```typescript
https://raw.githubusercontent.com/wiremoons/deno_mod/0.6.2/mod.ts
```
...
import {
  ...
  existsFile,
  isString,
} from "https://deno.land/x/deno_mod@0.6.2/mod.ts";
```
...
```typescript
import * as my_deno_mod from "https://deno.land/x/deno_mod@0.6.2/mod.ts";
```
