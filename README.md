...
used. In the example below the version tag `0.7.5` is specified.
...
tag** `0.7.5` which is the current version.
...
```typescript
https://deno.land/x/deno_mod@0.7.5/mod.ts
```
...
```typescript
https://raw.githubusercontent.com/wiremoons/deno_mod/0.7.5/mod.ts
```
...
```typescript
import * as my_deno_mod from "https://deno.land/x/deno_mod@0.7.5/mod.ts";
```
